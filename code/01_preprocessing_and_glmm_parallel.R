...
# Extract alcspp to model one specie only
m_alcspp_df <- eu_mammals_covs_values[["m_alcspp"]]

#Reference formula GLMM
full_model <- lme4::glmer(m_alcspp ~ Dvlpd_ZMean + RdDen10k_Zmean + Forest_ZMean + TRI10km_reSamp + I(TRI10km_reSamp^2) + Maxtmp10k_resamp + Wland_ZMean + Shrub10k_resamp + (1|biogeo),
                          data = m_alcspp_df, 
                          family = binomial)
...
best_model <- MuMIn::get.models(model_set, subset = 1)[[1]]
summary(best_model)
...
prediction_grid <- mammalscovs 
names(prediction_grid) 
model_cov <- c("Dvlpd_ZMean", "Forest_ZMean", "Maxtmp10k_resamp", "RdDen10k_Zmean", "Shrub10k_resamp",
                           "TRI10km_reSamp", "Wland_ZMean", "biogeo")

prediction_grid_sub <- terra::subset(prediction_grid, model_cov)
names(prediction_grid_sub)

# GGet coordenates to plot the predictions
dt <- terra::as.data.frame(prediction_grid_sub, xy = TRUE, na.rm = TRUE)

dt_rast <- terra::rast(dt, type="xyz")
plot(dt_rast)

# Predict using the model
predic_best_model <- terra::predict(prediction_grid_sub, best_model, type = "response", re.form = NA) # allow.new.levels = TRUE
# predic_avg <- terra::predict(evg_model, type = "response") # allow.new.levels = TRUE
plot(predic_best_model)

# Set the crs to the raster
crs(predic_best_model)  <- "epsg:3035"

writeRaster(predic_best_model, "alcspp_glmm_pred_habitat2.tif", overwrite=TRUE)



# GLMMs for ALL SPECIES -----------------------------------------------------

# Parallel process for GLMM models
...
species_models <- parLapply(cl, species_names, function(species_name) {
  # The species data has to be a list 
  species_data <- eu_mammals_covs_values[[species_name]]
  # run the model for each specie
...
model1 <- species_models[[1]]
best_model1 <- model1$best_model
summary(best_model1)
...
# save.image(file = "GLMM.RData")

## Predictions
predict_for_species <- function(fitted_model, species_name, prediction_grid) {
  # fitted model is a list containing the best model for each species
  best_model <- fitted_model$best_model
  
  # Generate predictions
  species_predictions <- terra::predict(prediction_grid, best_model, type = "response", re.form = NA)
  
  # set the CRS for the prediction raster
  crs(species_predictions) <- "epsg:3035"
  
  # Save the reuslt
  file_path <- paste0("predictions_2/", species_name, "_predictions.tif")
...
# Before ruining we have to alight the covariants to the covariants we used in the model
# Create a raste stack
prediction_grid <- mammalscovs 
...
# Test one specie
# model2 <- species_models[["m_canspp"]]
# summary(best_model2)
# 
# predict_for_species(model2, "m_canspp", eu_mammals_covs)


# Predictions in  a loop
# Create a list where all the paths 
predictions_paths <- list()

for (species_name in names(species_models)) {
  
  fitted_model <- species_models[[species_name]]
  # Run the predictions function to save the raster files
  predictions_paths <- predict_for_species(fitted_model, species_name, prediction_grid_sub)

  # Store the path of the saved prediction raster
  predictions_paths[[species_name]] <- predictions_paths
